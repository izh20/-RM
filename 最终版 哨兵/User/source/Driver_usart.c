...
extern PidTypeDef YawIPID,PitchIPID;   //航偏角位置外环
...
//   	 sendnum[2]=(uint8_t)((PitchOPID.target)&0XFF);
//  	 sendnum[3]=(uint8_t)((PitchOPID.target>>8)&0XFF);         w
//	 
...
//	sendnum[5]=(uint8_t)((RealAngleYAW>>8)&0XFF);
	...
	sendnum[3]=(uint8_t)((YAWANGLEMID>>8)&0XFF);	 
	 ...
	 sendnum[6]=(uint8_t)(((int)YawOPID.error)&0XFF);
	 sendnum[7]=(uint8_t)(((int)YawOPID.error>>8)&0XFF);
...
//	 sendnum[10]=(uint8_t)((Uart6_Lenth)&0XFF);
//	 sendnum[11]=(uint8_t)((Uart6_Lenth>>8)&0XFF);
	 
   
   
    sendnum[10]=(uint8_t)((HeadMove.PitchErr)&0XFF);
    sendnum[11]=(uint8_t)((HeadMove.PitchErr>>8)&0XFF);
