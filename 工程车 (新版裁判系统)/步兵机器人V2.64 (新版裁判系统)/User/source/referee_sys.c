...
static char cal_ref_data4(unsigned char *pdata, unsigned char nlen)
{
	if (verify_check(pdata, nlen))
	{
		...
		refSysData.PowerHeatData_t.ShooterHeat_17mm = (*(pdata + 21)) << 8 | *(pdata + 20);
		refSysData.PowerHeatData_t.ShooterHeat_42mm = (*(pdata + 23)) << 8 | *(pdata + 22);
        return 1;
...
static char cal_ref_data8(unsigned char *pdata, unsigned char nlen)
{
	if (verify_check(pdata, nlen))
	{
		...
		refSysData.GameRobotPos_t.yaw = hex_to_float(pdata + 19);
        return 1;
...
void uart_receive_idle(UART_HandleTypeDef *huart)
{
	...
	if ((__HAL_UART_GET_FLAG(huart, UART_FLAG_IDLE) != RESET))
	{
		if (huart->Instance == USART2)
		{
			...
			remian = hdma_usart2_rx.Instance->NDTR;
