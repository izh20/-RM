...
static void OneKeyDeal_WSADQR(uint16_t KeyPress)
{
    ...
    if(KeyPress&KEY_PRESSED_F)     //F:转换为红色识别
      ...
      set_red=0;
		...
		if(KeyPress&KEY_PRESSED_V)     //F:转换为红色识别
		{
      if(set_blue==0x00)
      {
        set_blue=0X01;
...
void MotorTargetChange(void)	
{
   ...
   static  int8_t ShootTwo=0;
	 //static char send=0;
      if(CatStruct.mode!=0x00)      //如果处于猫步模式
  ...
  if(RC_Ctl.rc.s1==0)  //遥控器模式
 ...
 else if(RC_Ctl.rc.s1==1)   //use mouse and keyboard to control
	{
		...
		TowKeyDeal_WSAD_T(RC_Ctl.key.v);  //处理组合按键
    ...
    if(RC_Ctl.mouse.press_l==1&&(RC_Ctl.rc.s2==3||RC_Ctl.rc.s2==2))
    {
      ...
      if((RC_Ctl.key.v&KEY_PRESSED_Z)||(RC_Ctl.key.v&KEY_PRESSED_X)||(RC_Ctl.mouse.press_r==1))      //如果V按键按下
      {
        
        if(RC_Ctl.key.v&KEY_PRESSED_X)
        {
          HeadMove.robots=0x00; //英雄识别模式
        }
        else
        {
...
//    
     else if(RC_Ctl.key.v&KEY_PRESSED_CTRL)       //如果CTR按键按下
      {
